...
namespace EdgeWiki.Parser
{
		...
		IComparable<StringSegment>
	{
		...
		{
			return string.Compare(segA.Owner, segA.m_nOffset, segB.Owner, segB.m_nOffset, Math.Min(segA.m_nLength, segB.m_nLength));
		}
		...
		{
			return string.Compare(segA.Owner, segA.m_nOffset, segB.Owner, segB.m_nOffset, Math.Min(segA.m_nLength, segB.m_nLength), sc);
		}
		...
		/// <param name="segA">The first segment.</param>
		/// <param name="segB">The second segment.</param>
		/// <param name="bIgnoreCase">If set to <c>true</c>, ignore case.</param>
		/// <param name="info">The culture info.</param>
		/// <returns>Zero, a positive integer, or a negative integer, if the first segment is
		/// equal to, greater than, or less than the second segment, respectively.</returns>
		public static int Compare(StringSegment segA, StringSegment segB, bool bIgnoreCase, CultureInfo info)
		{
			return string.Compare(segA.Owner, segA.m_nOffset, segB.Owner, segB.m_nOffset, Math.Min(segA.m_nLength, segB.m_nLength), bIgnoreCase, info);
		}
		...
		{
#if !MAC
			return string.CompareOrdinal(segA.Owner, segA.m_nOffset, segB.Owner, segB.m_nOffset, Math.Min(segA.m_nLength, segB.m_nLength));
#else
			return string.CompareOrdinal(segA.ToString(), segB.ToString());
#endif
		}
		...
		{
			int nIndex = m_strOwner.IndexOf(str, m_nOffset, m_nLength);
			return nIndex < 0 ? nIndex : nIndex - m_nOffset;
		}
		...
		public StringSegment Intersect(StringSegment seg)
		{
			if (m_strOwner != seg.m_strOwner)
				throw new ArgumentException("Segment from different string.", "seg");
		...
		{
			int nIndex = m_strOwner.LastIndexOf(str, m_nOffset + m_nLength - 1, m_nLength);
			return nIndex < 0 ? nIndex : nIndex - m_nOffset;
		}
...
#else
			// Mono doesn't correctly handle the ^ and $ symbols when given an explicit offset and length.
			// This is a sinful hack. I deserve to be punished.
			// What's worse, it *still* doesn't handle all of the possibilities--like a leading ^ after an
			// opening parenthesis; hopefully, it will be fixed (and released) in Mono before the next
			// release of Libronix.
			Match result = regex.Match(m_strOwner, m_nOffset, m_nLength);

			if (!result.Success)
			{
				Match intermediateResult = regex.Match(ToString());
				if (intermediateResult.Success)
				{
					// Remove leading ^ characters.
					string strSneakyPattern = regex.ToString().TrimStart('^');

					// Remove trailing, unescaped $ characters.
					Match patternMatch = Regex.Match(strSneakyPattern, @"(?<!\\(\\\\)*)\$+$");
					if (patternMatch.Success)
						strSneakyPattern = strSneakyPattern.Substring(0, patternMatch.Index);

					Regex reSneaky = new Regex(strSneakyPattern, regex.Options);
					result = reSneaky.Match(m_strOwner, m_nOffset + intermediateResult.Index, intermediateResult.Length);
				}
			}
		...
		{
			return m_strOwner.Substring(m_nOffset, m_nLength);
		}
